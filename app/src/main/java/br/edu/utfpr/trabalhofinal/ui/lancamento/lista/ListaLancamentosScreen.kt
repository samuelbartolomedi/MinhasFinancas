    ...
    LazyColumn(modifier = modifier) {
        items(lancamentos) { lancamento ->
            LancamentoItem(
                ...
                onClick = { onLancamentoPressed(lancamento) }
            )
        }
    }
}
...
fun LancamentoItem(
    ...
    val valorFormatado = lancamento.valor.formatar()
    val valorTexto = if (lancamento.tipo == TipoLancamentoEnum.DESPESA) "-$valorFormatado" else valorFormatado
